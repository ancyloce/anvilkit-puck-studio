...
export function Aside() {
    ...
    return (
        <div className="w-18 h-full py-4 border-r border-neutral-200 dark:border-neutral-800/80">
            <TooltipProvider>
                <Tabs value={activeTab} onValueChange={setActiveTab} orientation="vertical" className="w-full flex items-center justify-center">
                    <TabsList className="flex-col h-auto bg-transparent gap-2">
                        {tabs.map(({ value, icon, label }) => (
                            <Tooltip key={value}>
                                <TooltipTrigger asChild>
                                    <span>
                                        <TabsTrigger value={value} className="p-2">
                                            {icon}
                                        </TabsTrigger>
                                    </span>
                                </TooltipTrigger>
